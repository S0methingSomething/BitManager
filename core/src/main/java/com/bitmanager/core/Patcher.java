...
public class Patcher {
    ...
    public boolean patch(File inputApk, File outputApk, PatchConfig config) throws Exception {
        ...
        // Apply bsdiff patch to libil2cpp.so (pairip bypass)
        if (config.bsdiffPatch != null) {
            listener.onProgress("Applying bsdiff patch (pairip bypass)...");
            applyBsdiffPatch(apk, "lib/arm64-v8a/libil2cpp.so", config.bsdiffPatch);
            listener.onSuccess("Bsdiff patch applied");
        }
        
        // Apply native offset patches
        if (config.patches != null && !config.patches.isEmpty()) {
            listener.onProgress("Applying native patches...");
            applyNativePatches(apk, "lib/arm64-v8a/libil2cpp.so", config.patches);
    ...
    private void applyBsdiffPatch(ApkModule apk, String entryPath, File patchFile) throws Exception {
        ...
        byte[] patch = Files.readAllBytes(patchFile.toPath());
        
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        io.sigpipe.jbsdiff.Patch.patch(original, patch, out);
        byte[] patched = out.toByteArray();
        
        ByteInputSource newSource = new ByteInputSource(patched, entryPath);
        newSource.setMethod(ZipEntry.STORED); // .so files must be uncompressed
    ...
    private void applyNativePatches(ApkModule apk, String entryPath, java.util.List<NativePatch> patches) throws Exception {
        InputSource source = apk.getInputSource(entryPath);
        if (source == null) throw new IOException(entryPath + " not found in APK");
        
        byte[] data = readInputSource(source);
    ...
    private void applySmaliPatches(ApkModule apk, java.util.List<SmaliPatch> patches) throws Exception {
        // For now, smali patches are pre-compiled DEX patches
        // Future: integrate smali/baksmali for runtime compilation
        for (SmaliPatch patch : patches) {
            listener.onProgress("  • " + patch.name);
            // TODO: Implement smali patching with dexlib2
        }
    }
