...
public class Patcher {
    ...
    public boolean patch(File inputApk, File outputApk, PatchConfig config) throws Exception {
        // Use direct zip patching (works on Android without apktool)
        // Only use apktool for CoreX smali injection
        if (!config.corex) {
            return patchDirect(inputApk, outputApk, config);
        }
        
        // Full decompile/recompile for CoreX (requires apktool/java)
    ...
    private void signWithApksig(Path apk, String keystorePath) throws Exception {
        // Load keystore - use PKCS12 (works on Android, JKS doesn't)
        ...
        java.util.List<java.security.cert.X509Certificate> certs = java.util.Collections.singletonList(cert);
        
        // Use apksig via reflection - SignerConfig.Builder pattern
        ...
        Object builder = apkSignerBuilderClass
            .getConstructor(java.util.List.class)
            .newInstance(java.util.Collections.singletonList(signerConfig));
        
        apkSignerBuilderClass.getMethod("setInputApk", File.class).invoke(builder, apk.toFile());
        apkSignerBuilderClass.getMethod("setOutputApk", File.class).invoke(builder, apk.toFile());
        apkSignerBuilderClass.getMethod("setV1SigningEnabled", boolean.class).invoke(builder, true);
        apkSignerBuilderClass.getMethod("setV2SigningEnabled", boolean.class).invoke(builder, true);
        apkSignerBuilderClass.getMethod("setV3SigningEnabled", boolean.class).invoke(builder, false);
        
        Object signer = apkSignerBuilderClass.getMethod("build").invoke(builder);
